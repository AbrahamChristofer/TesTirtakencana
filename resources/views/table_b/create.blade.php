...
<div x-data="{ tab: 'form' }" class="max-w-2xl">
    ...
    <div x-show="tab === 'form'" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('table-b.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="nominal_transaksi" value="{{ old('nominal_transaksi') }}" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-400">
            </div>
    ...
    <div x-show="tab === 'excel'" x-cloak class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
        <form action="{{ route('table-b.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
            ...
            <div class="rounded-lg bg-slate-50 border border-slate-200 px-4 py-3 text-xs text-slate-600">
                ...
                <code class="bg-white px-2 py-1 rounded border border-slate-200">kode_toko | nominal_transaksi</code>
            </div>
            <div class="flex items-center gap-3 pt-2">
